...
// ⭐ دریافت تاریخ از پارامتر date (مثل روزانه)
...
$selected_year = $parts[0] ?? date('Y');
$selected_month = $parts[1] ?? date('m');

// ========== حذف ==========
if (isset($_GET['delete']) && !$is_readonly) {
    $stmt = mysqli_prepare($conn, "DELETE FROM income_monthly_records WHERE branch_id = ? AND record_year = ? AND record_month = ?");
    mysqli_stmt_bind_param($stmt, "iii", $branch_id, $selected_year, $selected_month);
    mysqli_stmt_execute($stmt);
    header('Location: monthly.php?date=' . $selected_date . ($branch_id != $user_id ? '&branch_id=' . $branch_id : ''));
    exit;
}
...
if (isset($_POST['save'])) {
    ...
    $count = 0;
    $stmt = mysqli_prepare($conn, "INSERT INTO income_monthly_records (branch_id, item_id, record_year, record_month, amount_gram, created_by) VALUES (?, ?, ?, ?, ?, ?)");
    foreach ($items as $item) {
        $item_id = intval($item['id']); 
        $gram = number_format(round(floatval($item['gram'] ?? 0), 3), 3, '.', '');
        if ($item_id > 0 && $gram > 0) {
            mysqli_stmt_bind_param($stmt, "iiiisi", $branch_id, $item_id, $year, $month, $gram, $user_id);
            if (mysqli_stmt_execute($stmt)) $count++;
        }
    }
    echo json_encode(['success' => true, 'message' => "✅ $count مورد"]);
    exit;
}
...
while ($r = mysqli_fetch_assoc($m_res)) $items[] = $r;

// ⭐ داده‌های ذخیره‌شده
$today_data = [];
$stmt = mysqli_prepare($conn, "SELECT * FROM income_monthly_records WHERE branch_id = ? AND record_year = ? AND record_month = ?");
mysqli_stmt_bind_param($stmt, "iii", $branch_id, $selected_year, $selected_month);
mysqli_stmt_execute($stmt);
$rec_res = mysqli_stmt_get_result($stmt);
while ($row = mysqli_fetch_assoc($rec_res)) $today_data[$row['item_id']] = $row;
...
<head>
    ...
    <title>درآمد ماهانه | <?php echo $branch_name; ?></title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .date-actions { display: flex; gap: 6px; margin-right: auto; }
        .btn-date-action {
        ...
        .btn-date-action.delete { 
            color: var(--red); 
            border-color: var(--red-bg); 
        }
        .btn-date-action.delete:hover {
            background: var(--red-bg);
            border-color: var(--red);
        }
        .btn-date-action svg { width: 14px; height: 14px; }
        ...
        .calc-btn:hover::before { opacity: 0.1; }
        .calc-btn:hover {
        ...
        .calc-btn:hover svg { transform: scale(1.1); }
        ...
        .calc-buttons .btn-op:hover { background: rgba(167,139,250,0.25); }
        .calc-buttons .btn-eq {
            background: var(--accent-bg);
            border-color: var(--accent);
            color: var(--accent);
        ...
        .calc-buttons .btn-eq:hover { background: rgba(212,175,55,0.25); }
        .calc-buttons .btn-clr {
            background: var(--red-bg);
            border-color: var(--red);
            color: var(--red);
        }
        .calc-buttons .btn-clr:hover { background: rgba(248,113,113,0.25); }
        ...
        .btn-apply:hover { background: rgba(212,175,55,0.25); }
        .btn-cancel {
            background: var(--bg-input);
            color: var(--text-2);
        }
...
<div class="wrapper">
    <div class="app-header">
        <div class="app-title">
            ...
            <span>درآمد ماهانه | <?php echo $branch_name; ?></span>
    ...
    ?>
            ...
            <a href="../index.php" class="logout-btn">بازگشت</a>
        </div>
    </div>
    ...
    <div class="tab-switch">
        ...
        <a href="../income/index.php?date=<?php echo $selected_date; ?>" 
           class="tab-switch-item <?php echo basename(dirname($_SERVER['SCRIPT_NAME'])) == 'income' && strpos($_SERVER['SCRIPT_NAME'], 'monthly') === false ? 'active' : ''; ?>">
            درآمد روزانه
        </a>
        <a href="../income/monthly.php?date=<?php echo $selected_date; ?><?php echo $branch_id != $user_id ? '&branch_id='.$branch_id : ''; ?>" 
           class="tab-switch-item active">
            درآمد ماهانه
        </a>
    ...
    <div class="totals-row">
        <div class="total-card">
            <div class="total-label">
                <svg style="width:14px;height:14px;vertical-align:middle;margin-left:4px;" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div class="total-value gram" id="totalGramMonthly">۰.۰۰۰ گرم</div>
        </div>
    </div>
    ...
    <div class="panel">
        <div class="panel-body">
            ...
            <input type="hidden" id="monthlyYear" value="<?php echo $selected_year; ?>">
            <input type="hidden" id="monthlyMonth" value="<?php echo $selected_month; ?>">
            ...
            <div class="date-bar">
                ...
                <span class="date-display"><?php echo $month_names[intval($selected_month)] . ' ' . $selected_year; ?></span>
                <input type="hidden" id="datepicker" class="date-input" value="<?php echo $selected_date; ?>" readonly style="width:140px;">
                <?php if (!$is_readonly): ?>
                <div class="date-actions">
                    <button class="btn-date-action delete" onclick="deleteMonth()">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <?php $currentCat = ''; foreach ($items as $item): 
                $val = $today_data[$item['id']] ?? null; 
                $gram = $val ? $val['amount_gram'] : '';
                ...
                if ($item['category'] && $item['category'] !== $currentCat): 
                    ...
                    echo '<div class="icard"><div class="icard-head">
                        ...
                        ' . $currentCat . '</div><div class="icard-body">';
                elseif (!$item['category'] && $currentCat): 
                    echo '</div></div></div>'; 
                    $currentCat = ''; 
                endif;
                if (!$item['category'] && !$currentCat && !isset($noCatM)): 
                    echo '<div class="icard"><div class="icard-head">
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            ?>
                <div class="item-row">
                    ...
                    <input type="text" class="monthly-input" data-id="<?php echo $item['id']; ?>" value="<?php echo $gram !== '' ? number_format((float)$gram, 3, '.', '') : ''; ?>" placeholder="گرم طلا" oninput="updateTotal(); debounceSave();" <?php echo $is_readonly ? 'disabled' : ''; ?>>
                    <?php if (!$is_readonly): ?>
            ...
            <?php endforeach; 
            if ($currentCat || isset($noCatM)) echo '</div></div></div>'; ?>
        </div>
    </div>
</div>
...
function changeBranch(id) { 
    window.location.href = 'monthly.php?branch_id=' + id + '&date=<?php echo $selected_date; ?>'; 
}

function deleteMonth() { 
    if (!confirm('حذف تمام رکوردهای این ماه؟')) return; 
    window.location.href = 'monthly.php?delete=1&date=<?php echo $selected_date; ?><?php echo $branch_id != $user_id ? '&branch_id='.$branch_id : ''; ?>'; 
}

// ========== Update Total ==========
function updateTotal() {
    var totalGram = 0;
    document.querySelectorAll('.monthly-input').forEach(function(inp) { 
        var gram = parseNum(inp.value);
        if (!isNaN(gram) && gram > 0) totalGram += gram;
    });
    
    var gramEl = document.getElementById('totalGramMonthly');
    if (gramEl) {
        gramEl.textContent = totalGram > 0 ? totalGram.toFixed(3) + ' گرم' : '۰.۰۰۰ گرم';
    }
}
...
async function saveData() {
    ...
    document.querySelectorAll('.monthly-input').forEach(function(inp) { 
        ...
        if (!isNaN(gram) && gram > 0) { 
            gram = Math.round(gram * 1000) / 1000;
            items.push({ id: parseInt(inp.dataset.id), gram: gram }); 
        } 
    });
    ...
    try { 
        var resp = await fetch('monthly.php', { 
            method:'POST', 
            headers:{'Content-Type':'application/x-www-form-urlencoded'}, 
            body:'save=1&year='+year+'&month='+month+'&items='+encodeURIComponent(JSON.stringify(items)) 
        }); 
...
function formatNumberWithCommas(numStr) {
    ...
    if (numStr.startsWith('-')) { isNegative = true; numStr = numStr.substring(1); }
...
function updateCalcDisplay() {
    ...
    if (calcResultShown) {
        ...
        var formattedParts = parts.map(function(part) {
            return ['+', '-', '*', '/', '×', '÷', '−'].includes(part) ? part : formatNumberWithCommas(part);
        });
...
function openCalc(btn) {
    ...
    calcTargetInput = row.querySelector('.monthly-input');
...
function calcClear() { calcExpression = '0'; calcResultShown = false; updateCalcDisplay(); }

function calcBack() {
    if (calcResultShown) { calcClear(); return; }
    if (calcExpression.length > 1) calcExpression = calcExpression.slice(0, -1);
    else calcExpression = '0';
    updateCalcDisplay();
}

function calcEqual() {
    try {
        var expr = calcExpression.replace(/,/g, '').replace(/×/g, '*').replace(/÷/g, '/').replace(/−/g, '-');
        if (/[^0-9+\-*/().]/.test(expr.replace(/\s/g, ''))) throw new Error('Invalid');
        var result = eval(expr);
        if (!isFinite(result)) calcExpression = 'خطا';
        else { result = Math.round(result * 100) / 100; calcExpression = result.toString(); }
    } catch(e) { calcExpression = 'خطا'; }
...
function handleCalcKeyboard(e) {
    ...
    else if (key === 'Backspace') calcBack();
    else if (key === 'Delete' || key === 'c' || key === 'C') calcClear();
...
$(document).ready(function() {
    $('#datepicker').persianDatepicker({
        ...
        onSelect: function(date) {
            window.location.href = 'monthly.php?date=' + date + '<?php echo $branch_id != $user_id ? '&branch_id='.$branch_id : ''; ?>';
        }
    });
