...
<div class="container">
    ...
    <div class="header">
        <h2>فرم ساز قرارداد تسهیلات ثنا</h2>
    ...
    <div class="card">
        ...
        <div class="form-row-2">
            ...
            <div><label>مشخصات دقیق وثیقه (ماده ۶)</label><input type="text" id="cCollateral" placeholder="مثال: یک عدد انگشتر نگین دار به وزن ۷.۱۷۰ گرم"></div>
        </div>
    ...
    <div class="card">
        ...
        <div class="form-row">
            <div><label>نرخ سود ماهانه (٪)</label><input type="number" id="cMonthlyRate" value="2" step="0.1" oninput="calcAll();"></div>
        ...
        <div class="form-row">
            <div class="summary-box"><b>مبلغ هر قسط</b> <span id="rInstAmount">۰</span> گرم</div>
...
function calcAll() {
    ...
    document.getElementById('rInstAmount').textContent = instGold.toFixed(3);
...
function openPrintTab() {
    ...
    let totalGold = cGoldRate > 0 ? total / cGoldRate : 0;
    let instGold = totalGold / cInstCount;
    ...
    for (let r = 0; r < rowsPerCol; r++) {
        ...
        for (let col = 0; col < 3; col++) {
            ...
            if (idx < cInstCount) {
                instTable += `<td>${idx+1}</td><td dir="ltr">${instDates[idx] || '—'}</td><td>${instGold.toFixed(3)} گرم</td>`;
            } else {
...
<body>
    <div class="page-container">
            ...
            <table class="header-table">
                <tr>
                    ...
                    <td class="header-center">
                        <h1>بسمه تعالی</h1>
                        <h2>« قرارداد اعطای تسهیلات وام ثنا »</h2>
                    </td>
        ...
        <div class="content">
            ...
            <p><b>ج) ضامن :</b> شرکت / آقای / خانم <b>${gName}</b> <b>فرزند:</b> ${gFather} <b>کد ملی:</b> ${gNational} <b>به نشانی:</b> ${gAddress} <b>تلفن:</b> ${gMobile} با امضای این قرارداد منفرداً و متفقاً کلیه مطالبات ناشی از گیرنده تسهیلات ذکر شده در بند ب ذیل ماده یک این قرارداد را پذیرفته و متعهد می گردد کلیه مطالبات معوق مربوط به قرارداد فوق در سررسید هر قسط با نرخ روز به حساب معرفی شده از سوی زرگری ثنا پرداخت نماید.</p>

            <div class="section-title">ماده ۲ : بهای معامله و شرایط پرداخت</div>
            <p>کل بهای فروش مورد معامله به متقاضی <b>${fmtNum(total)} ریال</b> به حروف (${numberToWords(total)} ریال) مشتمل بر قیمت تمام شده خرید مورد معامله توسط زرگری ثنا معادل <b>${fmtNum(cLoan)} ریال</b> به حروف (${numberToWords(cLoan)} ریال) و سود مربوطه معادل <b>${cMonthlyRate * 12} %</b> به مبلغ <b>${fmtNum(profit)} ریال</b> به حروف (${numberToWords(profit)} ریال) می‌باشد.</p>
            <p><b>تبصره ۱:</b> متقاضی متعهد می گردد در سررسید هر قسط، آنچه از بدهی، مطالبات و هزینه های مندرج در تبصره ۲ ذیل ماده ۲ باقی مانده است بر اساس نرخ روز مندرج در تابلو اعلام نرخ طلا بپردازد.</p>
            <p><b>تبصره ۲:</b> متقاضی متعهد می گردد تسهیلات دریافتی بر اساس هر گرم طلا <b>${fmtNum(cGoldRate)} ریال</b> به حروف (${numberToWords(cGoldRate)} ریال) در تاریخ صدور قرارداد به میزان کل <b>${totalGold.toFixed(3)} گرم</b> طلا و به اقساط مساوی <b>${instGold.toFixed(3)} گرم</b> طلا را طبق تبصره یک ذیل ماده ۲ تسویه نماید.</p>
            ...
            <p>متقاضی متعهد می گردد در صورت تخطی از مفاد این قرارداد به هر عنوان عین وثیقه دریافت شده بدون مراجعه به مراجع قضائی به دین حال تبدیل شده و قرارداد فوق به صورت یک طرفه فسخ و کلیه مطالبات این قرارداد با جرایم مربوطه به مأخذ ۶٪+ از عین وثیقه با نرخ روز طلا کسر گردد.</p>

            <div class="section-title">ماده ۶ : وثایق و تضامین</div>
            ...
            <b>${cCollateral}</b></p>
            <p><b>تبصره:</b> زرگری ثنا متعهد می گردد عین وثیقه موصوف در ماده ۶ را پس از تسویه کامل قرارداد فوق به صاحب آن وام گیرنده / ضامن مسترد نماید.<br>
            نام و نام خانوادگی وثیقه گذار: <b>${gName}</b> | کد ملی: <b>${gNational}</b> | فرزند: <b>${gFather}</b> | تلفن: <b>${gMobile}</b><br>
            آدرس: ${gAddress}</p>

            <div class="section-title">ماده ۷ : شرایط نقدی تسهیلات</div>
            <p>چنانچه وام گیرنده وجه ریالی تسهیلات اعطائی را درخواست نماید، بابت تبدیل طلای آبشده معادل ۱ درصد از اصل وام به عنوان کارمزد فروش به بنکداری طلا کسرخواهد شد.</p>

            <div class="section-title">ماده ۸ : فسخ وام قبل از پایان قرارداد</div>
            ...
            <p><b>تبصره ۲:</b> چناچه وام گیرنده بعد از سپری شدن یک ماه و قبل از اتمام قراداد مربوطه نسبت به فسخ وام اقدام نماید، کسر ماه به صورت روز شمار محاسبه خواهد شد.</p>
            <p><b>تبصره ۳:</b> در زمان فسخ قرارداد قبل از موعد نهایی، بابت مانده بدهی قرارداد، نسبت به خرید طلای آبشده با نرخ روز اقدام و یک درصد کارمزد اخذ خواهد شد.</p>
            <p><b>تبصره ۴:</b> در زمان تسویه بخشی از اقساط وام، سود اقساط مربوطه کسر و مانده بدهی تبدیل به قرارداد جدید خواهد شد و بابت خرید طلای آبشده به میزان تسویه اقساط، مشمول ۱ درصد کارمزد می باشد.</p>

            <div class="section-title">ماده ۹ : نسخ قرارداد</div>
            <p>این قرارداد در ۹ ماده و ۷ تبصره و در ۲ نسخه تنظیم که هر دو حکم واحد را دارند، به رویت کامل زرگری ثنا، متقاضی، ضامن و وثیقه گذار رسید و ایشان ضمن اقرار به اطلاع و آگاهی کامل از مفاد این قرارداد و پذیرش آن به امضای تمامی صفحات آن مبادرت نمودند و یک نسخه از این قرارداد به هریک از آنها تسلیم گردید.</p>
            ...
            <div style="margin: 4mm 0; padding: 3mm; border: 1px dashed #666; background: #fafafa; line-height: 2;">
                ...
                لطفا مبلغ <b>........................................</b> ریال معادل وزن طلای <b>..................</b> گرم از این قرارداد را به صورت ریالی به حساب شماره <b>..............................................................</b> اینجانب <b>..............................</b> واریز فرمایید.
            ...
            <p style="text-align:center; font-weight: bold; margin-bottom: 4mm;">شماره تلفن همراه آقای/خانم: ${cName} (${cMobile}) &nbsp;&nbsp;&nbsp; | &nbsp;&nbsp;&nbsp; مقدار هر قسط: ${instGold.toFixed(3)} گرم</p>
